...
describe('procedures store CRUD', () => {
  ...
  it('listProcedures with type filter returns matching type', () => {
    ...
    store.putProcedure(
      makeProc({ id: 'p4', statement: 'Use monospace fonts in editor', type: prefType }),
    );
  ...
  it('listProcedures with flaggedOnly filter returns flagged procedures', () => {
    store.putProcedure(
      makeProc({ id: 'p7', statement: 'Outdated deployment flow', flaggedForReview: true }),
    );
    store.putProcedure(
      makeProc({ id: 'p8', statement: 'Current deployment flow', flaggedForReview: false }),
    );
...
describe('applyProcedure confidence dynamics', () => {
  ...
  it('positive feedback increments applications', () => {
    store.putProcedure(
      makeProc({ id: 'c-pos-app', statement: 'Cache expensive calls', applications: 2 }),
    );
  ...
  it('positive feedback increases stability', () => {
    store.putProcedure(
      makeProc({ id: 'c-pos-stab', statement: 'Use feature flags', stability: 1.1 }),
    );
  ...
  it('negative feedback decreases confidence by factor 0.8', () => {
    store.putProcedure(
      makeProc({ id: 'c-neg-conf', statement: 'Always squash merge', confidence: 0.5 }),
    );
  ...
  it('negative feedback increments contradictions', () => {
    store.putProcedure(
      makeProc({ id: 'c-neg-ctr', statement: 'Use strict linting', contradictions: 1 }),
    );
...
describe('recall integration with procedures', () => {
  ...
  before(async () => {
    ...
    store.putNode({
      ...
      excerpts: [
        { file: 'deploy.md', text: 'Deploy workflow and rollback checklist', date: '2026-01-15' },
      ],
    });
